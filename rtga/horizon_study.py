...
def diagnostic_figure(output):
    ...
    fig.suptitle("Selected diagnostic failures · observed paths before actions 0–79; crosses mark goals",fontsize=11)
    fig.tight_layout()
...
def write_report(output, paper):
    ...
    if "confirmation" in stages:
        ...
        report += [""]
        s = summaries(stages["confirmation"]["records"])
        repeat = effects["repeat4"]["mean_distance"]
        longer = effects["horizon48"]["mean_distance"]
        report += ["## Interpretation", "",
                   f"The selected failure rescue did not become a higher observed success rate on new seeds: original persistence and all three variants reached {s['original']['successes']}/{s['original']['n']} goals. Repeated initialization reduced mean distance by {-repeat[0]:.4f} world units (variant-minus-original interval [{repeat[1]:+.4f}, {repeat[2]:+.4f}]), with no resolved improvement in first-hit time or final distance. This is a small unadjusted secondary effect, not a demonstrated general repair.", "",
                   f"The H48/N32 configuration increased mean distance by {longer[0]:.4f} and used {s['horizon48']['latency_p50_ms']/s['original']['latency_p50_ms']:.2f}× the median planning time. It did get closer to the goal on the shared failed seed 217, which helps explain its lower final-distance mean, but did not turn that approach into success within the fixed action budget. More lookahead is not free, and this width-for-depth exchange did not improve the overall primary outcome.", "",
                   f"Fresh evolution reached {s['fresh']['successes']}/{s['fresh']['n']} goals here, despite rescuing both selected old failures. Resetting the population can escape some routes and lose useful plans elsewhere. Keep the original planner as a comparison and preserve repeated initialization as a cheap ablation; this one comparison does not establish a replacement default. No further tuning or confirmation sweep was performed.", ""]
    ...
    for r in stages["diagnostic"]["records"]:
        ...
        report.append(f"| {r['seed']} / {r['condition']} | {r['first_success_step'] if r['success'] else 'no hit'} | {d['longest_wall_contact_run']} | {d['first_observed_crossing_step']} | {d['last_observed_position']} |")
    report += ["", "Wall contact uses the observed puck center at the collision boundary outside the traversable doorway. Trace diagnostics cover pre-action states 0–79; the outcome table includes the final 80th transition. Entropy in the raw diagnostics describes only the 24 stored score-ordered genomes, not full-population diversity. Compressed `.json.gz` traces and standalone HTML viewers are stored for both diagnostic seeds and preselected confirmation seeds 200/201.", "", "## Provenance and limits", "",
    ...
    report += [f"The exact numerical source is preserved in [source-snapshot]({os.path.relpath(output / 'source-snapshot', paper.parent)}). Later changes to the live runner concern report wording; the archived file matches the recorded phase hashes.", ""]
    replays_path = output / "confirmation/explanatory-replays.json"
    if replays_path.exists():
        replays = json.loads(replays_path.read_text())
        report += [f"Seven post-hoc explanatory replays preserve all five conditions on seed 217 and original/fresh on seed 206. Their actions and distances exactly match the existing confirmation records. These are not seven additional independent samples; the extra rendering runs used {sum(r['real_actions'] for r in replays['records']):,} real actions and {sum(r['model_transitions'] for r in replays['records']):,} model transitions, separate from the comparison. [Replay ledger]({os.path.relpath(replays_path, paper.parent)}).", ""]
    for phase,data in stages.items():
