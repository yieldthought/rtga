...
def fixture_trace():
    ...
    return {
        ...
        "environment": {"width": 1, "height": 1, "wall_x": .6, "door_y": [.4, .6],
                        "switch": [.25, .7], "noise_source": [.8, .2], "variant": "fixture"},
        "frames": [
...
for (const path of process.argv.slice(1)) for (const width of [360,1200]) {
  ...
  const script=source.match(/<script>(.*?)<\/script>/s)[1],elements=new Map();
  const c=new Proxy({}, {get:(target,key)=>target[key]||((...args)=>{for(const x of args)if(typeof x==='number')assert.ok(Number.isFinite(x),key+' had nonfinite coordinates');}),set:(target,key,value)=>{target[key]=value;return true;}});
